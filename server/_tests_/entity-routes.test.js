...
describe('Entity Endpoints', () => {
  ...
  it('POST / Should return validation error message if missing required properties', async () => {
    const res = await request.post('/carryme')
      .send(mockEntity[1]);
  ...
  it('PUT /:id should update an entity', async () => {
    await Entity.insertMany(mockEntity[0]);
    const entities = await request.get('/carryme');
    const entityID = entities.body[0]._id;
  ...
  it('PUT /:id should return "Validation failed" if invalid entry', async () => {
    await Entity.insertMany(mockEntity[0]);
    const entities = await request.get('/carryme');
    const entityID = entities.body[0]._id;
